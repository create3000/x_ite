...
X_ITE supports a limited set of nodes defined by the X3D specification. There are currently 243 of 263 nodes (89 %) implemented. The implementation of these nodes is complete in that that the nodes will support all the required fields and features for that implementation unless otherwise stated.
