...
**NormalInterpolator** generates a series of normal (perpendicular) 3-tuple SFVec3f values. Each vector value has unit length, i.e. the set of allowed vectors describes the surface of a unit sphere. Authors can ROUTE value_changed output events to a [Normal](/x_ite/components/rendering/normal/) node's vector field, or to another 3-tuple SFVec3f field.
