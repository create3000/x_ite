...
- *name* prefix must match ancestor [HAnimHumanoid](../hanim/hanimhumanoid) *name* followed by underscore character, if more than one humanoid appears within a scene file. For example, 'Nancy_' prepended before location *name*.
...
Defines index values into the parent [HAnimSegment](../hanim/hanimsegment) or HAnimBody/[HAnimHumanoid](../hanim/hanimhumanoid) coordinate array for the mesh of vertices affected by this HAnimDisplacer. Values start at index 0.
...
*displacements* are a set of SFVec3f values added to neutral/resting position of each of the corresponding [HAnimSegment](../hanim/hanimsegment) vertices (or [HAnimJoint](../hanim/hanimjoint)/[HAnimHumanoid](../hanim/hanimhumanoid) vertices) referenced by coordIndex field.

#### Hints

- Individual displacement values are scaled by the weight factor, if present.
- Since default pose faces along +Z axis, -x values are right side and +x values are left side within [HAnimHumanoid](../hanim/hanimhumanoid).

## Advice

### Hints

- HAnimDisplacer can be used in three different ways: (a) identify vertices corresponding to a particular feature in a parent [HAnimSegment](../hanim/hanimsegment) node, (b) represent a particular muscular action for a parent [HAnimJoint](../hanim/hanimjoint) node by displacing corresponding [HAnimHumanoid](../hanim/hanimhumanoid) skin vertices in various directions (linearly or radially), or (c) represent a complete configuration of coordinate vertices in parent [HAnimSegment](../hanim/hanimsegment) or [HAnimJoint](../hanim/hanimjoint) nodes. Example: in the case of a face, there might be a separate HAnimDisplacer node for each facial expression.
- Multiple HAnimDisplacer nodes must appear consecutively inside parent [HAnimSegment](../hanim/hanimsegment) for proper content validation in XML encoding.
...
- Allowed name suffixes include _feature, _action and _config.
- Index values for [HAnimHumanoid](../hanim/hanimhumanoid) skin [IndexedFaceSet](../geometry3d/indexedfaceset), skinCoord and skinNormal nodes must all be consistently defined together with [HAnimJoint](../hanim/hanimjoint) [HAnimSegment](../hanim/hanimsegment) and HAnimDisplacer nodes for proper skin animation.
