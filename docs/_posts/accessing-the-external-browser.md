...
Sometimes it is needed to react on output events generated by a node. It is possible to connect a callback function to an output field to handle these events.

```js
// Add a field callback to be notified when touchTime is fired. "check" is an
// arbitrary string to identify the callback, especially if you want to
// remove the callback later.
touchSensor .addFieldCallback ("check", "touchTime", value =>
{
