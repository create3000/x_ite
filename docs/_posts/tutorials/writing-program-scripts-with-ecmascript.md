...
A program script implements the [Script](../components/scripting/script) node using values from the interface:
...
- the [Script](../components/scripting/script) node is created (typically when the browser loads the world)
...
- the [Script](../components/scripting/script) node is deleted
...
- A [TouchSensor](../components/pointingdevicesensor/touchsensor) node can supply touch events
