...
- You can specify your own normals with a [Normal](../components/rendering/normal) node
...
Normals can be given for [IndexedFaceSet](../components/geometry3d/indexedfaceset) and [ElevationGrid](../components/geometry3d/elevationgrid) nodes.
...
A [NormalInterpolator](../components/interpolation/normalinterpolator) node describes a normal set:

- *keys* - key fractions
- *keyValues* - key normal lists (X,Y,Z lists)
- Interpolates lists of normals, similar to the [CoordinateInterpolator](../components/interpolation/coordinateinterpolator)
...
Typically route into a [Normal](../components/rendering/normal) node's set\_vector input.

## Summary

- The *creaseAngle* field controls faceted or smooth shading
- The [Normal](../components/rendering/normal) node lists normal vectors to use for parts of a shape
...
- The [NormalInterpolator](../components/interpolation/normalinterpolator) node converts times to normals
