...
A String value containing the URL against which relative URLs are resolved. By default, this is the address of the web page itself. Although this feature is rarely needed, it can be useful when using a `data:` URL with the *src* attribute or with `Browser.loadURL` when using the external browser.
