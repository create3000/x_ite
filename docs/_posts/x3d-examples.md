...
An X3D galaxy animation can be efficiently implemented using [InstancedShape](/x_ite/components/x-ite/instancedshape/)  together with a [ComposedShader](/x_ite/components/shaders/composedshader/) to render thousands of stars with minimal geometry overhead. In this approach, a single star mesh (for example a small quad or point sprite) is reused through InstancedShape, where per-instance attributes such as position, color, brightness, and orbital radius are passed to the GPU. The [ComposedShader](/x_ite/components/shaders/composedshader/) then computes the dynamic motion of each star in the vertex shader, typically applying a spiral rotation around the galactic center using time-based uniforms and mathematical functions to produce swirling arms. Because the animation is performed directly on the GPU, the galaxy can contain many thousands of animated stars while maintaining interactive performance in the X3D viewer.
