...
This X3D example demonstrates a galaxy animation implemented with [InstancedShape](/x_ite/components/x-ite/instancedshape/) and a [ComposedShader](/x_ite/components/shaders/composedshader/) to efficiently render and animate a large number of stars. A single base geometry representing a star is replicated using [InstancedShape](/x_ite/components/x-ite/instancedshape/), allowing thousands of instances to share the same mesh. The [ComposedShader](/x_ite/components/shaders/composedshader/) controls the animation on the GPU, using time-based uniforms and mathematical transformations to rotate the instances around a central galactic core and to form spiral arms. In the vertex shader, star positions are updated to create orbital motion and subtle vertical variation, while the fragment shader can adjust brightness and color to simulate stellar diversity and depth. By performing the animation in the shader rather than through CPU-driven updates, the scene achieves smooth real-time motion and scalability, making it possible to visualize a dynamic spiral galaxy with many thousands of stars in an interactive X3D environment.
