...
X3D (function ()
{
  ...
  var Browser = X3D .getBrowser ("X3DCanvas.browser");
...
var x3dcanvases = document .querySelectorAll ("X3DCanvas");

for (var i = 0; i < x3dcanvases .length; ++ i)
{
  // Obtain X3DBrowser object of element i.
  var Browser = X3D .getBrowser (x3dcanvases [i]);
  ...
}
```
...
function addBrowser (parent)
{
   ...
   var x3dcanvas = X3D .createBrowser ();
...
function foo (node)
{
  ...
  var type = node .getNodeType () .reverse ();
